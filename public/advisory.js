    ...
    try {
      ...
      if (t.output && t.output.suggestions) {
        html += '<div class="detail-list" style="margin-top:16px">' + t.output.suggestions.map(function (s, i) {
          return '<div><dt>建议 ' + (i + 1) + ' · ' + escapeHtml(s.targetField) + ' · <span class="status ' + (s.confidence === 'high' ? 'ready' : s.confidence === 'medium' ? 'caution' : 'blocked') + '">' + escapeHtml(s.confidence) + '</span></dt><dd><strong>' + escapeHtml(s.proposedValue) + '</strong></dd><dd style="color:var(--muted);font-size:13px;margin-top:4px">' + escapeHtml(s.rationale) + '</dd>'
