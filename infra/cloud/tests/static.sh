...
require_text "$dockerfile" 'ARG NODE_BASE_IMAGE=node:24-bookworm-slim'
require_text "$dockerfile" 'FROM ${NODE_BASE_IMAGE} AS build'
require_text "$dockerfile" 'FROM ${NODE_BASE_IMAGE}'
