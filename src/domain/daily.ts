...
export const dailyInputSchema = z.object({
  ...
  stageGoal: z.string().trim().min(2).max(500),
  mainContradiction: z.string().trim().min(2).max(1000),
  bottleneck: z.string().trim().min(2).max(1000),
  mainAction: z.string().trim().min(2).max(1000),
  deliverable: z.string().trim().min(2).max(1000),
  estimatedMinutes: z.coerce.number().int().min(1).max(1440),
  stopCondition: z.string().trim().min(2).max(1000),
