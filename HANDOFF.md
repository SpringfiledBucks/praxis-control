...
- 整改、标准服务端部署和完整 Web 优先于继续扩展 Windows/Linux 原生 GUI；完整原生客户端暂缓；
- 轻量 PGlite 本机服务与全量 PostgreSQL 云端服务继续共用领域规则、Web、API 和审计，但任何时刻只有一个权威事实源；
- Windows 11 小组件作为独立薄入口继续规划，第一阶段优先验证稳定 HTTPS Web/PWA 小组件；它只读取最小摘要并打开 Web，不复制业务规则或执行高风险动作；
- Windows 安装、启动、更新、小组件和故障处理不得要求用户关闭或削弱 Microsoft Defender、SmartScreen、防火墙或第三方安全软件，也不得要求添加目录/进程排除项；兼容性问题必须通过签名、可信发布、最小权限、标准 HTTPS 和可审计诊断解决；
- 周复盘统计允许用户修改，但系统计算快照、人工调整、原因和最终报告值必须分层保存并追加审计，不能覆盖原始事实；
- 可以开始规划大模型接入，首批只做结构化建议、复盘草稿、事实检索和规则解释；模型不能决定硬门槛、静默写入正式事实或自动执行外部动作；
- 具体整改顺序、标准云部署、小组件和模型接入门槛见 `docs/plans/2026-07-31-remediation-service-web-widget-llm.md`。
- 标准单节点云部署代码基线已加入 `infra/cloud/`：独立一次性迁移、单应用副本、回环发布、只读/降权容器和存活/就绪双探针；当前仍需 Gitea CI 与隔离容器验收，真实域名、证书、digest 镜像、备份和恢复演练未完成前不得称为生产可用。

## 当前外部状态

- NAS Gitea 私有仓库已建立，本地唯一 `origin` 通过受控网络连接该权威远端，`main` 已推送；内部地址、仓库路径和账号不进入公开文档；
- Gitea Actions 基础设施与最新业务能力 run 87 的 `verify`、`linux-gui-smoke`、`postgres-contract` 三个作业全部通过，见 `docs/status/2026-07-29-gitea-ci.md`、`docs/status/2026-07-30-business-logic-foundation.md` 与 `docs/status/2026-07-30-decision-workflow-and-guide.md`；
